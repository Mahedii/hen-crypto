...
<head>
  ...
  transform: scale(8,8);
    ...
    <title>Versekerings huis inhoud</title>
    <?php include 'layouts/title-meta.php'; ?>
...
$sql = "SELECT * FROM versekering_images";
...
if (isset($_SERVER['REQUEST_METHOD'])) {
    if (isset($_POST["edit"])) {
        ...
        $id = $_POST["id"];
        $value_name = "value_" . $id;
        $description_name = "description_" . $id;
        $value = isset($_POST[$value_name]) ? $_POST[$value_name] : '';
        $description = isset($_POST[$description_name]) ? $_POST[$description_name] : '';


        $stmt = mysqli_prepare($conn, "UPDATE versekering_images SET value=?, description=?, 'serial=? WHERE id=?");
        mysqli_stmt_bind_param($stmt, "ssi", $value, $description, $serial, $id);
        ...
        $id = $_POST["id"];
        $sql = "DELETE FROM versekering_images WHERE id=$id";
    ...
    if (isset($_POST['edit'])) {
        ...
        $sql = "UPDATE versekering_images SET value='$value', description='$description', serial='$serial' WHERE id=$id";
				...
				<div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">Huis Inhoud</h4>
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="javascript: void(0);">Alle huis inhoud war verseker is</a></li>
                                    <li class="breadcrumb-item active">Huis Inhoud</li>
                                </ol>
                            </div>
		...
		<table>
			<thead>
				<tr>
					...
					<th>Beskrywing</th>
					<th>Prent</th>
					<th>Naam van Reeks#</th>
					<th>foto van Reeks#</th>
				...
				<?php
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td><img src='" . $row["second_image_location"] . "'></td>";
                            echo "<td>";
...
?>
		...
		var valueInput = row.querySelector("input[name='value_" + id + "']");
		...
		valueInputPopup.value = currentValue;
		...
		saveButton.onclick = function() {
		  ...
		  var newValue = valueInputPopup.value;
		  ...
		  xhr.send("id=" + id + "&edit=true&value_" + id + "=" + newValue + "&description_" + id + "=" + newDescription);
		...
		<div class="popup" id="editPopup">
			<div class="popup-content">
				<h2>Edit Record</h2>
				<form>
					<input name="id" type="hidden"> <label for="value">Value:</label> <input name="value" type="text"> <label for="description">Description:</label> <input name="description" type="text">
